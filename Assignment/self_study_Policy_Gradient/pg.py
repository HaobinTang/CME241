...
class PG(object):
  ...
  def build_policy_network_op(self, scope = "policy_network"):
    ...
    if self.discrete:
      action_logits = build_mlp(self.observation_placeholder, self.action_dim, "discrete", config.n_layers,config.layer_size )           # TODO
      self.sampled_action =  tf.squeeze(tf.multinomial(action_logits, 1), axis=1)   # TODO
      self.logprob =  -tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.action_placeholder, logits=action_logits)        # TODO
    else:
      action_means = build_mlp(self.observation_placeholder, self.action_dim, "continuous",config.n_layers,config.layer_size)         # TODO
  ...
  def add_baseline_op(self, scope = "baseline"):
    ...
    self.baseline = tf.squeeze(build_mlp(self.observation_placeholder, 1, scope, config.n_layers,config.layer_size )) # TODO
  ...
  def get_returns(self, paths):
    ...
    for path in paths:
      ...
      #########   YOUR CODE HERE - 5-10 lines.   ############
      n = len(rewards)
      returns = [np.sum(np.power(config.gamma, np.arange(n-t)) * rewards[t:]) for t in range(n)]# TODO
